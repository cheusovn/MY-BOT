...
async def _send_lead_magnet(chat_id: int):
    """Отправляет лид-магнит серией сообщений — экспертный контент с конкретикой."""

    # Часть 1: Вступление
    await bot.send_message(chat_id,
        "🎁 <b>Как я автоматизировал свой Instagram\n"
        "8 AI-агентами — без команды и бюджета</b>\n\n"
        "Это не теория. Это работающая система,\n"
        "которая прямо сейчас ведёт мой аккаунт.\n\n"
        "Ниже — конкретные инструменты, промпты\n"
        "и настройки. Сохрани и пользуйся 👇"
    )

    # Часть 2: Агент-аналитик (конкретика)
    await bot.send_message(chat_id,
        "📊 <b>АГЕНТ 1: Аналитик</b>\n\n"
        "<b>Что делает:</b> каждый понедельник в 9:00\n"
        "собирает отчёт за неделю.\n\n"
        "<b>Инструменты:</b>\n"
        "• Instagram Graph API — охваты, показы, клики\n"
        "• OpenRouter API (GPT-4o-mini) — анализ данных\n\n"
        "<b>Готовый промпт:</b>\n"
        "<code>Проанализируй метрики Instagram за неделю:\n"
        "[вставь данные]\n"
        "Выдели: топ-3 поста по охвату, худший пост\n"
        "и почему, рекомендации на следующую неделю.\n"
        "Формат: таблица + 3 вывода.</code>\n\n"
        "💡 <i>Результат: я трачу 0 минут на аналитику,\n"
        "но точно знаю что работает.</i>"
    )

    # Часть 3: Копирайтер (промпт)
    await bot.send_message(chat_id,
        "✍️ <b>АГЕНТ 4: Копирайтер</b>\n\n"
        "<b>Что делает:</b> пишет хуки, описания и хештеги\n"
        "под каждый пост за 30 секунд.\n\n"
        "<b>Промпт для хука (копируй):</b>\n"
        "<code>Напиши 5 хуков для Instagram-поста.\n"
        "Тема: [тема]\n"
        "Ниша: нейросети и AI для бизнеса.\n"
        "Аудитория: предприниматели 25-40 лет.\n"
        "Стиль: дерзкий, с провокацией.\n"
        "Формат: первая строка — хук (до 7 слов),\n"
        "вторая — раскрытие (1 предложение).\n"
        "Пример хорошего хука:\n"
        "«Твой SMM-щик стоит 60 000₽. Мой — 0₽.»</code>\n\n"
        "💡 <i>За месяц агент написал 120+ хуков.\n"
        "Лучшие дали x3 к обычным охватам.</i>"
    )

    # Часть 4: Дизайнер (конкретный стек)
    await bot.send_message(chat_id,
        "🎨 <b>АГЕНТ 6: Дизайнер каруселей</b>\n\n"
        "<b>Стек:</b>\n"
        "• Nano Banana 2 (Gemini 3.1) через OpenRouter\n"
        "• Референс-стиль + промпт = готовый слайд\n"
        "• Формат: 1080×1350 (4:5, больше места в ленте)\n\n"
        "<b>Как это работает:</b>\n"
        "1. Отправляю референс-картинку + фото автора\n"
        "2. Пишу промпт с текстом слайда\n"
        "3. Получаю готовый слайд за 15 секунд\n"
        "4. Стоимость: ~$0.07 за слайд (~7₽)\n\n"
        "<b>Промпт-шаблон:</b>\n"
        "<code>Instagram carousel slide 1080x1350.\n"
        "Style: reference image. Black bg #0a0a0a,\n"
        "lime accent #c8ff00, bold condensed font.\n"
        "Title: [ЗАГОЛОВОК НА РУССКОМ]\n"
        "Elements: 3D mockup, glass cards, glow.</code>\n\n"
        "💡 <i>10 слайдов карусели = 70₽ и 10 минут.\n"
        "Дизайнер берёт 5 000–15 000₽ за то же самое.</i>"
    )

    # Часть 5: Воронка (схема)
    await bot.send_message(chat_id,
        "🔄 <b>АГЕНТ 7: Воронка продаж</b>\n\n"
        "<b>Моя схема (работает прямо сейчас):</b>\n\n"
        "Карусель с пользой\n"
        "  ↓ CTA: «пиши АГЕНТЫ»\n"
        "Автоответ в комментариях\n"
        "  ↓ + DM со ссылкой\n"
        "Telegram-бот (ты сейчас тут)\n"
        "  ↓ проверка подписки на канал\n"
        "Лид-магнит (этот гайд)\n"
        "  ↓ знакомство с курсом\n"
        "2 бесплатных дня курса\n"
        "  ↓ результат за 2 дня\n"
        "Покупка полного курса\n\n"
        "<b>Конверсия:</b> ~3-5% от комментария до покупки.\n"
        "При 100 комментах = 3-5 продаж × 2 970₽ = 9-15K₽\n\n"
        "💡 <i>Вся воронка работает автоматически 24/7.\n"
        "Я не отвечаю на комменты руками.</i>"
    )
    ...
    await bot.send_message(chat_id,
        "⚡ <b>Итого: 8 агентов заменяют:</b>\n\n"
        "• SMM-щика — 40-80K₽/мес\n"
        "• Дизайнера — 30-60K₽/мес\n"
        "• Копирайтера — 20-40K₽/мес\n"
        "• Аналитика — 30-50K₽/мес\n\n"
        "Мои расходы: <b>~500₽/мес</b> на API.\n\n"
        "🔥 <b>Хочешь собрать такую же систему?</b>\n"
        "На курсе TRUE AI ACADEMY за 7 дней\n"
        "покажу всё по шагам — от нуля до результата.\n\n"
        "Первые 2 дня — бесплатно, без карты 👇",
