...
async def cb_special_tariffs(call: CallbackQuery):
    ...
    text = (
        ...
        "Первые 2 дня бесплатны. Не подойдёт после оплаты —\n"
        "вернём <b>100% в первый день</b>. Без вопросов.\n\n"
        f"{social_proof()}\n\n"
        "👇 Выбирай:"
    )
...
async def cb_tariffs(call: CallbackQuery):
    ...
    text = (
        ...
        "━━━━━━━━━━━━━━━━\n"
        "🛡 <b>Гарантия возврата:</b>\n"
        "Не подойдёт после оплаты — вернём <b>100% в первый день</b>.\n\n"
        "👇 Выбери тариф:"
    )
    await show(call, text, tariffs_kb(s))
...
async def cb_faq(call: CallbackQuery):
    text = (
        ...
        "+ Гарантия: вернём 100% оплаты в первый день.\n\n"
        "━━━━━━━━━━━━━━━━\n"
...
async def cb_guarantee(call: CallbackQuery):
    text = (
        ...
        "▸ Не подойдёт после оплаты — напиши менеджеру\n"
        "▸ Вернём <b>100% оплаты</b> в первый день\n"
        "▸ Доступ к материалам остаётся у тебя\n\n"
        "━━━━━━━━━━━━━━━━\n"
        "<b>Почему мы так уверены?</b>\n\n"
        "Сначала ты 2 дня бесплатно делаешь реальный\n"
        "результат — и только потом решаешь, платить ли.\n"
        "Поэтому остаются те, кому курс действительно зашёл.\n\n"
...
async def cb_buy(call: CallbackQuery):
    ...
    text = (
        ...
        "🛡 Гарантия возврата действует при любом способе.\n"
            ...
            try:
                ...
                # FU3: tariffs, не купил — через 20ч
                    ...
                    await bot.send_message(
                        ...
                        "Риска нет: вернём 100% оплаты в первый день.\n\n"
                        f"{social_proof()}\n\n"
                        "👇 Выбрать:",
